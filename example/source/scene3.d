...
Scene createScene3() {
    class Scene3 : Scene {
        ...
        this() {
            ...
            static class ModelMaterial : Material {
                mixin VertexShaderSource!q{
                    ...
                    void main() {
                        gl_Position = projectionMatrix * viewMatrix * worldMatrix * vec4(position.xzy, 1);
                        normal2 = (worldMatrix * vec4(normal,0)).xyz;;
                    }
                };
                ...
                mixin FragmentShaderSource!q{
                    ...
                    void main() {
                        fragColor = vec4(normalize(normal2) * .5 + .5, 1);
                    }
                };
            }
            
            static class M : Model {
                mixin ImplPos;
                mixin ImplRot;
                mixin ImplScale;
                mixin ImplWorldMatrix;
                mixin Material!(
                    MaterialPattern!(ModelMaterial, ".*"));
                mixin ImplLoad;
            }

            auto model = M.load("resource/test.blend");
            model.scale = 0.1;
